		...
		<div class="jm-dashboard__actions">
			<?php if ( job_manager_user_can_submit_job_listing() ) : ?>
				<a class="jm-ui-button"
					href="<?php echo esc_url( get_permalink( $submit_job_form_page_id ) ); ?>"><span><?php esc_html_e( 'Add Ride', 'wp-job-manager' ); ?></span></a>
			<?php endif; ?>
		</div>
	</div>
				...
				<?php echo Notice::dialog(
					[
						'message' => $search_input
							...
							: __( 'You do not have any active rides.', 'wp-job-manager' )
					]
				); ?>
			</div>
		<?php else : ?>
